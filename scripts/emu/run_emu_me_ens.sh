...
${COMMAND} ${RCP} -cmd=mwt_ens -in_sample=samples_mwt/ensemble_all_${FINAL_STATE}.txt -name=${FINAL_STATE}_all_ens -out_root=tt_${FINAL_STATE}_ens_all_${SUFFIX}.root -out_ascii=tt_${FINAL_STATE}_ens_all_${SUFFIX}.txt -perensemble=${NPERENS} -nensembles=500 -title=tt_${FINAL_STATE}_ens_all_${SUFFIX}
